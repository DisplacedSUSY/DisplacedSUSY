...
def makeSRyieldsTable(year):
    ...
    signalEvents = Variable("Events in signal", is_independent=False, is_binned=False, units="")
    ...
    dataEvents = Variable("Events in data", is_independent=False, is_binned=False, units="")
    dataEvents.values = obs["y"]
    dataUncert = Uncertainty("68% CL", is_symmetric=False)
    dataUncert.values = obs["dy"]
    dataEvents.add_uncertainty(dataUncert)
    dataEvents.add_qualifier("SQRT(S)", 13, "TeV")

    backgroundEvents = Variable("Background events", is_independent=False, is_binned=False, units="")
...
def makeStopPaperGraphLimitsTable(decay):
    ...
    obsLimits = Variable("Observed 95% CL upper limits on $m_{\\tilde{t}}$", is_independent=False, is_binned=False, units="GeV")
    obsLimits.values = obs["x"]
    obsLimits.add_qualifier("SQRT(S)", 13, "TeV")

    expLimits = Variable("Expected 95% CL upper limits on $m_{\\tilde{t}}$", is_independent=False, is_binned=False, units="GeV")
...
def makeStop3ChannelLimitsTable(decay):
    ...
    for i,channel in enumerate(channelExpNums):
        if channel=="7":
            channelName = "channel combination"
        elif channel=="9":
            channelName = "e$\\mu$ channel"
        elif channel=="11":
            channelName = "ee channel"
        elif channel=="13":
            channelName = "$\\mu\\mu$ channel"

        expLimits = Variable("Expected 95% CL upper limits on $m_{\\tilde{t}}$, "+channelName, is_independent=False, is_binned=False, units="GeV")
    ...
    for i,channel in enumerate(channelObsNums):
        ...
        obsLimits = Variable("Observed 95% CL upper limits on $m_{\\tilde{t}}$, "+channelName, is_independent=False, is_binned=False, units="GeV")
...
def makeSignificanceTable(channel):
    ...
    table.keywords["observables"] = ["Significance, $c\\tau_{0}$, top squark mass"]
