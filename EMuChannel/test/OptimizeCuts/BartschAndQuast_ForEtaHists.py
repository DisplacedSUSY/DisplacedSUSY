...
CanvasEle.SetLogy()
#FrameEle = CanvasEle.DrawFrame(xhiValues[0],0,xhiValues[29], 300) #linear
FrameEle = CanvasEle.DrawFrame(xhiValues[0],0.001,xhiValues[29], 30000) #log
...
for signal in range(len(signalDataset)):
    legEle.AddEntry(HistEles[signal],str(signalDataset[signal]),"p")
    HistEles[signal].Draw("PEL")
legEle.Draw()
CanvasEle.SaveAs("EtaOpt_BartschAndQuast_Ele.pdf")
...
CanvasMu.SetLogy()
#FrameMu = CanvasMu.DrawFrame(xhiValues[0],0,xhiValues[29],300) #linear
FrameMu = CanvasMu.DrawFrame(xhiValues[0],0.001,xhiValues[29],30000) #log
...
CanvasMu.cd()
for signal in range(len(signalDataset)):
    legMu.AddEntry(HistMus[signal],str(signalDataset[signal]),"p")
    HistMus[signal].Draw("PEL")
legMu.Draw()
CanvasMu.SaveAs("EtaOpt_BartschAndQuast_Mu.pdf")
