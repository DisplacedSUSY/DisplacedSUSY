...
#python BartschAndQuast_ForPtHists.py -l options.py -w DisplacedLeptons2016/Preselection_PtCutOptimization
import time
import os
import sys
import math
...
signalDataset = []
histEle = "Electron Plots/electronPt"
histMu = "Muon Plots/muonPt"


for dataset in datasets:
    #if dataset=="Background":
    if dataset=="MuonEG_2016_2017_2018":
        ...
        #print "    electron integral from " + str(xlo) + " GeV to 2000 GeV is: " + str("%.1f" % eleInt) + " +/- " + str("%.1f" % eleIntError)
        #print "    muon integral from " + str(xlo) + " GeV to 2000 GeV is: " + str("%.1f" % muInt) + " +/- " + str("%.1f" % muIntError)
...
for signal in range(len(allSignalEleInts)):
    ...
    for bkgdEleInt,signalEleInt in zip(bkgdEleInts,allSignalEleInts[signal]):
        BartschAndQuastEle = 2.*(sqrt(signalEleInt+bkgdEleInt)-sqrt(bkgdEleInt))
        BartschAndQuastEles.append(BartschAndQuastEle)
    for bkgdMuInt,signalMuInt in zip(bkgdMuInts,allSignalMuInts[signal]):
        BartschAndQuastMu = 2.*(sqrt(signalMuInt+bkgdMuInt)-sqrt(bkgdMuInt))
        BartschAndQuastMus.append(BartschAndQuastMu)

    HistEle = ROOT.TGraphAsymmErrors(len(xloValues),xloValues,BartschAndQuastEles)
    HistEle.SetTitle("")
    ...
    print "The optimal electron |d0| is: "+str("%.3f" % xloValues[maxIndexEle]) + " GeV for "+ str(signalDataset[signal])

    HistMu = ROOT.TGraphAsymmErrors(len(xloValues),xloValues,BartschAndQuastMus)
    HistMu.SetTitle("")
    ...
    print "The optimal muon |d0| is: "+str("%.3f" % xloValues[maxIndexMu]) + " GeV for "+ str(signalDataset[signal])
    print "-----------"
...
FrameEle = CanvasEle.DrawFrame(0,0,xloValues[499], 600)
FrameEle.GetXaxis().SetTitle("Minimum electron p_{T} [GeV]")
FrameEle.GetYaxis().SetTitle("Q = 2(#sqrt{S+B}-#sqrt{B})")
...
legEle.Draw()
CanvasEle.SaveAs("BartschAndQuast_ElePt_linear_highMass.pdf")
...
FrameMu = CanvasMu.DrawFrame(0,0,xloValues[499],600) #linear
FrameMu.GetXaxis().SetTitle("Minimum muon p_{T} [GeV]")
...
legMu.Draw()
CanvasMu.SaveAs("BartschAndQuast_MuPt_linear_highMass.pdf")
