...
def GetYieldAndErrorDir(process,d0cut):
    ...
    xCutBin = d0Histogram.GetXaxis().FindBin(float(d0cut))    
    yCutBin = d0Histogram.GetYaxis().FindBin(float(d0cut))    
    xUpperCutBin = d0Histogram.GetXaxis().FindBin(float(d0UpperCut))    
    yUpperCutBin = d0Histogram.GetYaxis().FindBin(float(d0UpperCut))    

    normIntErr = Double (0.0)
    normIntegral = d0Histogram.IntegralAndError(xCutBin, xUpperCutBin - 1, yCutBin, yUpperCutBin - 1, normIntErr)  
...
for dataset in datasets:
    ...
    for d0cut in d0cuts_array:
        ...
        yieldAndError = {}
        if arguments.method == "Para":
            yieldAndError = GetYieldAndErrorPara(dataset,d0cut)
        if arguments.method == "Dir":
            yieldAndError = GetYieldAndErrorDir(dataset,d0cut)
...
outputFile = condor_dir + "/targetRegionYields_" + plainTextString(channel) + "_" + arguments.method + ".tex"
