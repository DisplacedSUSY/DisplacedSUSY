...
def customizeProduce (process):
    ...
    process.leptonicTauDecayGenFilter = cms.EDFilter("LeptonicTauDecayGenFilter",
                                                     inputTag = cms.InputTag("genParticlePlusGeant")
                                                   )

    if hasattr (process, "simulation_step") and hasattr(process, "psim"):
      getattr(process, "simulation_step")._seq = getattr(process,"simulation_step")._seq * process.genParticlePlusGeant * process.leptonicTauDecayGenFilter
