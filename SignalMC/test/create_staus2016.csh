...
#foreach i (150 250 350 450) #staus then maybe do these
   foreach j (0p01 0p1 1 10 100 1000) #prioritize these
   #foreach j (10000) #then maybe do these
	cmsDriver.py Configuration/Generator/python/Staus_M_${i}_${j}mm_TuneCUETP8M1_13TeV_pythia8_cff.py --fileout file:staus${i}_${j}mm.root --mc --eventcontent RAWSIM --customise SLHCUpgradeSimulations/Configuration/postLS1Customs.customisePostLS1,SimG4Core/CustomPhysics/Exotica_HSCP_SIM_cfi.customise,DisplacedSUSY/SignalMC/genParticlePlusGeant2016.customizeKeep,DisplacedSUSY/SignalMC/genParticlePlusGeant2016.customizeProduce --datatier GEN-SIM --conditions MCRUN2_71_V1::All --beamspot Realistic50ns13TeVCollision --step LHE,GEN,SIM --magField 38T_PostLS1 --no_exec
	sed -i "s/SelectEvents = cms.vstring('generation_step')/SelectEvents = cms.vstring('simulation_step')/" Staus_M_${i}_${j}mm_TuneCUETP8M1_13TeV_pythia8_cff_py_LHE_GEN_SIM.py
